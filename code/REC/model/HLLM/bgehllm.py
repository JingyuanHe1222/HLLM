...
class BGEHLLM(BaseModel):
    ...
    def __init__(self, config, dataload):
        ...
        if self.item_emb_token_n > 0: # torch.Size([1, 1, 2048])
            self.item_emb_tokens = nn.Parameter(
                torch.zeros(1, self.item_emb_token_n, self.item_llm.config.hidden_size) # [1, 1, 2048]
            )
            self.item_emb_tokens.data.normal_(mean=0.0, std=0.02) # gaussian init 
            if config['item_emb_pretrain']: # repro: do not enter 
    ...
    def nce_loss(self, cur_embs, target_pos, target_neg, user_attention_mask):
        ...
        D = target_neg.size(-1)
        # cosine sim of positive embeddings and predicted embeddings -> s(E' j,i, E j,i)
        output_embs = cur_embs / cur_embs.norm(dim=-1, keepdim=True) # [bz, num_neg, hd]
        target_pos_embs = target_pos / target_pos.norm(dim=-1, keepdim=True) # [bz, num_neg, hd]
        pos_logits = F.cosine_similarity(output_embs, target_pos_embs, dim=-1).unsqueeze(-1) # [bz, num_neg, 1]
        # cosine sim of negative embeddings and predicted embeddings -> s(E' j,i, E' j,i,k)
        target_neg = target_neg / target_neg.norm(dim=-1, keepdim=True)
        neg_embedding_all = all_gather(target_neg, sync_grads=True).reshape(-1, D)  # [num, hd]
        neg_embedding_all = neg_embedding_all.transpose(-1, -2)
        neg_logits = torch.matmul(output_embs, neg_embedding_all) # [bz, num_neg, num]
        fix_logits = torch.matmul(target_pos_embs, neg_embedding_all) # [bz, num_neg, num]
        neg_logits[fix_logits > self.nce_thres] = torch.finfo(neg_logits.dtype).min

        logits = torch.cat([pos_logits, neg_logits], dim=-1) # [bz, num_neg, num+1]
        logits = logits[user_attention_mask.bool()] * logit_scale # concat the bz*num_neg dim -> [num_unmasked, num]
    ...
    def forward_item_emb(
        ...
        # just get embed_tokens weights on pos_input_ids (all pos inputs concatenated)
        inputs_embeds = llm.get_input_embeddings()(input_ids) # [input_ids.shape[0], 2048]
        emb_pos = cu_input_lens.cumsum(dim=0, dtype=torch.int32) # -> all pos input concatenated, the pos of the end of each
        if emb_token_n > 0: # 1
            inputs_embeds[emb_pos - 1] = emb_tokens # sub the last [item] token to trainable embed  
    ...
    def forward(self, interaction, mode='train'):
        ...
        user_attention_mask = interaction['attention_mask'] # attention mask use for user 
